...
const keyWords = ['Bedrijfsnaam', 'Bezoekers', 'Reden'];
...
function updateAppointments() {
  ...
  googleCalendar.getCalendar(date.start, date.end)
    ...
    .then(newEvents => {
      ...
      console.log(`${newEvents.length} appointments added`);
  ...
  events.forEach(event => {
    appointments.forEach((appointment, i) => {
      if(event.id === appointment.googleId && event.updated !== appointment.lastModified) {
        console.log(`Appointment: ${event.summary} updated`);
...
function getInformation(event) {
  if(event.description === undefined) {
    console.log(`No description in event: ${event.summary}`);
    return [];
  }
